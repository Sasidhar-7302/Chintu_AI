...
def handle_open_app(text: str, context: Dict[str, Any]) -> ActionResult:
    ...
    if app:
        if app_discovery.open_app(app):
            ...
            get_state_manager().set_last_opened_app(app.name)
    ...
    for site_name, url in KNOWN_WEBSITES.items():
        if site_name in target or target in site_name:
            webbrowser.open(url)
    ...
    if "battery" in text_lower:
        try:
            ...
            if battery:
                ...
                return ActionResult.ok(
                    f"Battery is at {percent}% and {plugged}.",
                    {"battery": percent, "plugged": battery.power_plugged},
                    "system_info"
                )
            else:
                return ActionResult.fail("No battery detected. This might be a desktop.", "system_info")
        except ImportError:
            return ActionResult.fail("Battery info not available.", "system_info")
    
    # WiFi status
    if "wifi" in text_lower or "internet" in text_lower or "connection" in text_lower:
        import socket
        ...
        try:
            ...
            return ActionResult.ok("You're connected to the internet.", {"connected": True}, "system_info")
        except OSError:
...
def handle_note_taking(text: str, context: Dict[str, Any]) -> ActionResult:
    ...
    if any(phrase in text_lower for phrase in ["take a note", "save a note", "note that"]):
        ...
        return ActionResult.ok("Got it! I've saved that note.", {"note_id": note_id}, "note_taking")
...
def register_core_capabilities() -> None:
    ...
    registry.register(Capability(
        name="screen_control",
        triggers=["click", "type", "write", "dictate", "scroll", "move mouse", "press", "enter text"],
    ...
    registry.register(Capability(
        ...
        triggers=["what time", "what's the time", "current time", "what date", "today's date", 
                  "battery", "wifi status", "internet connection"],
...
def handle_smart_reading(text: str, context: Dict[str, Any]) -> ActionResult:
    ...
    from ..browser.smart_reader import get_smart_reader
    import asyncio
...
def handle_browse_url(text: str, context: Dict[str, Any]) -> ActionResult:
    ...
    from ..web.url_reader import get_url_reader
    import re
...
def handle_clipboard(text: str, context: Dict[str, Any]) -> ActionResult:
    ...
    from ..platform.clipboard import get_clipboard
...
# Register
try:
    from ..llm.reasoning import DeepReasoner
    registry = get_registry()

...
class SystemIntegrator:
    ...
    def initialize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                from ..automation.platform import get_platform
                ...
                # Surface device availability to UI
                try:
                    from .state import get_state_manager
                    sm = get_state_manager()
                    caps = self.platform_detector.capabilities
                    sm.update_feature(
                        "microphone",
                        enabled=caps.has_microphone,
                        status="active" if caps.has_microphone else "inactive",
                        error=caps.errors.get("microphone") if not caps.has_microphone else None,
                    )
                    sm.update_feature(
                        "speaker",
                        enabled=caps.has_speaker,
                        status="active" if caps.has_speaker else "inactive",
                        error=caps.errors.get("speaker") if not caps.has_speaker else None,
                    )
                    sm.update_feature(
                        "camera",
                        enabled=caps.has_camera,
                        status="active" if caps.has_camera else "inactive",
                        error=caps.errors.get("camera") if not caps.has_camera else None,
                    )
                    if self.error_reporter:
                        from .error_reporter import ErrorSeverity
                        if not caps.has_microphone:
                            self.error_reporter.report_error(
                                Exception(caps.errors.get("microphone", "No microphone detected")),
                                severity=ErrorSeverity.WARNING,
                                component="microphone",
                                user_message="Microphone not detected. Voice input may not work.",
                            )
                        if not caps.has_speaker:
                            self.error_reporter.report_error(
                                Exception(caps.errors.get("speaker", "No speakers detected")),
                                severity=ErrorSeverity.WARNING,
                                component="speaker",
                                user_message="Speakers not detected. Voice output may not work.",
                            )
                        if not caps.has_camera:
                            self.error_reporter.report_error(
                                Exception(caps.errors.get("camera", "No camera detected")),
                                severity=ErrorSeverity.WARNING,
                                component="camera",
                                user_message="Camera not detected. Gesture features will be disabled.",
                            )
                except Exception as e:
                    logger.debug(f"State update for platform capabilities failed: {e}")
            except Exception as e:
            ...
            try:
                try:
                    spec = importlib.util.find_spec("chintu_backend.distributed.state_sync")
                except ModuleNotFoundError:
                    spec = None
                if spec is None:
                    logger.info("Distributed state module not available; skipping")
                else:
    ...
    def _on_health_change(self, health):
        ...
        try:
            sm = get_state_manager()
            status = "active" if health.status.value == "healthy" else "inactive"
            sm.update_feature(
                health.component,
                enabled=(health.status.value == "healthy"),
                status=status,
                error=None if health.status.value == "healthy" else health.message,
            )
        except Exception as e:
            logger.debug(f"Failed to update UI health feature: {e}")

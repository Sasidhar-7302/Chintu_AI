...
class WakeWordDetector:
    ...
    def __init__(
        ...
        match_threshold: float = 0.75,  # Lowered from 0.88 for better tolerance
        ...
        stt_confidence_threshold: float = 0.4,  # Lowered from 0.6 for noisy environments
        ...
        if HAS_OPENWAKEWORD:
            ...
            if not wake_supported:
                ...
                # User explicitly wants custom wake word only - skip base model fallbacks.
                self._allow_openwakeword = False
                logger.info(
                    "Wake word '%s' not supported by openWakeWord; using STT fallback only (no base model).",
                    self.wake_word,
                )
        ...
        try:
            ...
            if self._model_path:
                if os.path.exists(self._model_path):
                    ...
                    # Do not fall back to base models when custom model is missing.
                    self._allow_openwakeword = False
                    return
            else:
                ...
                if candidate in openwakeword.MODELS:
                    ...
                    # No supported wake word model; skip base fallback entirely.
                    self._using_custom_model = False
                    self._allow_openwakeword = False
                    return
    ...
    def _normalize_wake_transcript(self, text: str) -> str:
        ...
        cleaned = text.lower()
        replacements = {
            # Common Whisper mishearings of "hey chintu"
            "it's in two": "hey chintu",
            "its in two": "hey chintu",
            "it's into": "hey chintu",
            "its into": "hey chintu",
            "itsinto": "hey chintu",
            "it sin two": "hey chintu",
            "fetch into": "hey chintu",
            "hinching to": "hey chintu",
            "h2": "hey chintu",
            "a chin two": "hey chintu",
            "a chin to": "hey chintu",
            # Name variants
            "chintoo": "chintu",
            "chin tu": "chintu",
            "chin to": "chintu",
            ...
            "chint u": "chintu",
            # More edge cases
            "hey gent you": "hey chintu",
            "a gent you": "hey chintu",
            "agent you": "hey chintu",
            "hey gentle": "hey chintu",
            "shin to": "chintu",
            "shin tu": "chintu",
            "shinto": "chintu",
        }
    ...
    def _confirm_wake_word(self, audio: np.ndarray, model_name: str):
        try:
            ...
            logger.info(f"[Wake STT] Transcript: '{text}' (conf={confidence:.2f})")
            normalized = self._normalize_wake_transcript(text)
            logger.info(f"[Wake STT] Normalized: '{normalized}'")
            
            if not self._is_valid_wake_transcript(text, confidence):
                logger.info(f"[Wake STT] Rejected - low confidence or invalid")
                return
            
            # Check if transcript matches wake phrase
            matches = self._matches_wake_phrase(text)
            logger.info(f"[Wake STT] Matches wake phrase: {matches}")
            ...
            has_token = self._has_wake_token(text)
            logger.info(f"[Wake STT] Has wake token: {has_token}")
            
            if has_token:
                logger.info(f"[Wake STT] Confirmed via wake token in: '{text}'")
                self._trigger_wake(model_name, confirmed=True)
                return

            logger.info(f"[Wake STT] REJECTED - no wake phrase match in: '{text}'")
        except Exception as exc:
    ...
    def _transcribe_fallback(self, audio: np.ndarray) -> tuple[str, float]:
        ...
        try:
            # Use initial_prompt to bias Whisper toward the wake phrase
            initial_prompt = "Hey Chintu. Hey chintu. Chintu."

...
class SpeechToText:
    ...
    def __init__(
        ...
        vad_filter: bool = False,  # Disabled - was removing valid speech
    ...
    def _load_model(self):
        ...
        if HAS_FASTER_WHISPER:
            try:
                ...
                # === NEW: Smart VRAM Optimization ===
                if self.device == "auto":
                    try:
                        from chintu_backend.swarm.vram_monitor import get_vram_monitor
                        vram = get_vram_monitor().get_status()
                        
                        # We need about 1.5GB free for safe GPU usage of small model
                        # "small.en" takes ~600MB with int8_float16, but we need overhead
                        if vram.free_mb > 1500:
                            self.device = "cuda"
                            # Force memory-efficient compute type on GPU
                            compute_type = "int8_float16"
                            logger.info(f"Smart VRAM: Using GPU (Free: {vram.free_mb}MB)")
                        else:
                            self.device = "cpu"
                            compute_type = "int8"
                            logger.info(f"Smart VRAM: Using CPU (Free: {vram.free_mb}MB < 1500MB)")
                    except Exception as e:
                        logger.warning(f"Smart VRAM check failed: {e} - defaulting to CPU")
                        self.device = "cpu"
                        compute_type = "int8"
                ...
                try:
                    ...
                    if self.device == "cuda":
                        logger.warning(f"CUDA failed (missing libraries?): {cuda_err}. Falling back to CPU.")
                        self.device = "cpu"
                        compute_type = "int8"
                        self._model = WhisperModel(
                            self.model_name,
                ...
                logger.info(
                    "Loaded faster-whisper model: %s (device=%s, compute_type=%s, cpu_threads=%s)",
                    self.model_name,
                    self.device,
                    compute_type,
    ...
    def set_silence_threshold(self, threshold: float):
        """Update silence threshold based on ambient noise calibration."""
        try:
            value = float(threshold)
        except Exception:
            return
        self._silence_threshold = max(0.0, min(1.0, value))
    ...
    def _transcribe(self, audio: np.ndarray, is_partial: bool = False) -> str:
        ...
        try:
            ...
            err_msg = str(e).lower()
            # Catch persistent CUDA/DLL errors that initialization might have missed
            if "cuda" in err_msg or "cublas" in err_msg or "cudnn" in err_msg or "library not found" in err_msg:
                if self.device != "cpu":
                    logger.warning(f"[STT] Late CUDA error during transcription: {e}. Attempting emergency CPU fallback...")
                    self.device = "cpu"
                    self._compute_type = "int8"
                    try:
                        self._load_model()
                        # Retry transcription on CPU
                        return self._transcribe(audio, is_partial)
                    except Exception as fatal:
                        logger.error(f"Emergency STT fallback failed: {fatal}")
